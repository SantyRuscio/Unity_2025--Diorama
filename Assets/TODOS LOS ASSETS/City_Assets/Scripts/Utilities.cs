...
public class UTUUtilities : AssetPostprocessor
{
	...
	[MenuItem( "UTU/ImportTerrainHeightmap" )]
	static void ImportTerrainHeightmap()
	{
		Terrain[] AllTerrains = UnityEngine.Object.FindObjectsOfType<Terrain>();
		...
		Terrain SelectedTerrain = AllTerrains[0];
		UnityEngine.Object Obj = AssetDatabase.LoadAssetAtPath<Texture>( "Assets/LandscapeTestHeightmap_0.png" );
		if( Obj == null )
			return;
		...
		if ( Tex != null )
		{
            ...
            for(int x = 0; x < Tex.width; x++ )
                for( int y = 0; y < Tex.height; y++ )
				{
                    ...
                    if ( MirrorX )
					{
						ActualX = ( Tex.width - 1 ) - x;
                    }
                    if( MirrorY )
                    {
                        ActualY = ( Tex.height - 1 ) - y;
                    }
                    heights[ActualX, ActualY] = ( h - 0.5f );
                }
             SelectedTerrain.terrainData.SetHeights( xBase, yBase, heights );
        }
    }
}
#endif